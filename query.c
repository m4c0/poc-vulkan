//
// query.c - Example for query APIs
//
...
static VkShaderModule vlk_create_shader_module() {
  FILE * f = fopen("query.comp.spv", "rb");
  assert(f);
...
static void vlk_allocate_memories() {
  ...
  VkMemoryAllocateInfo info = {
    ...
    .allocationSize = sizeof(float) * 1024 * 1024,
...
int main() {
  ...
  vkCmdResetQueryPool(vlk_cb, vlk_qpool, 0, 5);
  vkCmdWriteTimestamp(vlk_cb, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, vlk_qpool, 0);
  vkCmdDispatch(vlk_cb, 1, 1, 1);
  vkCmdWriteTimestamp(vlk_cb, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, vlk_qpool, 1);
  vkCmdDispatch(vlk_cb, 1, 1, 1);
  vkCmdWriteTimestamp(vlk_cb, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, vlk_qpool, 2);
  vkCmdDispatch(vlk_cb, 1024, 1, 1);
  vkCmdWriteTimestamp(vlk_cb, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, vlk_qpool, 3);
  vkCmdDispatch(vlk_cb, 1024, 1, 1);
  vkCmdWriteTimestamp(vlk_cb, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, vlk_qpool, 4);
  ...
  uint64_t data[5];
  _(vkGetQueryPoolResults(vlk_dev, vlk_qpool, 0, 5, 5 * sizeof(uint64_t), data, sizeof(uint64_t), VK_QUERY_RESULT_64_BIT));
  printf("Timestamps:\n");
